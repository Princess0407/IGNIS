...
void onReceive(const esp_now_recv_info *info, const uint8_t *data, int len) {
  ...
  Serial.printf("[NodeC] Relayed msg from %s | ID:%u | Fire:%d | Hops left:%d\n",
...
void calibrateBaseline() {
  Serial.println("[NodeC] Calibrating baseline (10s)...");
  long sum = 0;
...
void setup() {
  ...
  if (esp_now_init() != ESP_OK) {
    Serial.println("[NodeC] ESP-NOW init FAILED");
    return;
  }
...
void loop() {
  ...
  Serial.printf("[NodeC] Raw:%d  EMA:%.1f  Baseline:%.1f  Threshold:%.1f\n",
  ...
  if (emaValue > fireThreshold && !fireState) {
    ...
    Serial.printf("[NodeC] 🔥 FIRE sent | MsgID:%u\n", msg.messageID);
  }
    ...
    Serial.printf("[NodeC] ✅ SAFE sent | MsgID:%u\n", msg.messageID);
  }
